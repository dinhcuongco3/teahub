...
describe('DateSelector.vue', () => {
  ...
  it('updates selected date when a MyDate component emits newValue', async () => {
    const monthInput = wrapper.findComponent({ ref: 'month' })
    const dayInput = wrapper.findComponent({ ref: 'day' })
    const yearInput = wrapper.findComponent({ ref: 'year' })
  ...
  it('emits a new valid date when all inputs are filled and valid', async () => {
    ...
    expect(wrapper.emitted().newDate[2]).toEqual(['2023-05-12'])
  })
